...
主题**不会自动替你创建菜单、资源中心、工具箱或关于页**，这是为了避免将 AIYM 的站点结构写进每位用户的新站。启用后请按下列顺序完成初始化。

### 1. 配置 Halo 主菜单（决定桌面与移动端显示什么）

1. Halo 后台 → **外观 → 菜单**，编辑或新建「主菜单」。
2. 将你已经发布的页面或链接加入菜单，例如：`首页`、`文章`、`归档`、`关于`。
3. **资源中心、工具箱、关于页只有在你自己创建并加入主菜单后才会出现**；未创建时不影响主题使用。
4. 菜单图标可在菜单项的自定义属性中填 `icon`（Iconify 图标名，如 `ri-home-line`）。

> 自适应规则：桌面端会按实际可用宽度把末尾菜单收进「•••」；移动端不超过 4 项时全部显示，超过时保留前 4 项，第 5 格为「更多」。

### 2. 配置首页文章来源（推荐）

Halo 后台 → **外观 → 主题设置 → 内容设置 → 博客文章分类**，选择你的文章根分类。

- 已选择：首页只显示该分类及其子分类的文章；
- 未选择：首页自动显示全部已发布文章，**不会空白**。

### 3. 仅在需要时创建扩展页面

| 需要的能力 | 后台操作 | 是否必须 |
|---|---|---|
| 资源中心 | 新建单篇页面，模板选 `resources`；再在主题「内容设置」选择资源分类 | 否 |
| 工具箱 | 新建单篇页面，路径建议为 `tools`，模板选 `toolbox` | 否 |
| 关于页 | 新建单篇页面，模板选 `about` | 否 |
| 在线工具 | 新建单篇页面，路径以 `tools/` 开头，模板选 `tool-detail` | 否 |

资源分类未设置时，资源中心会展示明确的后台配置提示；工具箱会自动保留主题默认的 3 个外部实用链接（网络测速、图片压缩、ITDOG），你可在「主题设置 → 工具箱」修改或删除。

### 4. 系统页面与插件页面

| 页面 | 状态 | 说明 |
|---|---|---|
| 首页、文章、独立页面 | 已适配 | 含深浅色、搜索、响应式布局与底部导航。 |
| 分类列表 / 分类详情、标签、归档、404 / 500 | 已适配 | 主题内含对应模板。 |
| 图库 / 相册、瞬间 | 不提供独立主题模板 | 这两类通常由 Halo 插件提供路由和模板；本主题仅将 Halo 图库用作无封面文章、资源的随机封面来源。安装对应插件后，请以插件的主题兼容说明为准。 |
...
[MIT License](https://github.com/myaiym/theme-aiym/blob/main/LICENSE)
